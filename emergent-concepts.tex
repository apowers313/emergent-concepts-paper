...
\subsection{Matching}
Matching is a synonym for recognition or recall --- it reads the ontology graph and returns the best fitting Concept and / or Context for the information that has been made available by saliency and cross-modal attention. If a match is found for the existing data, it is routed to Sculpting mechanisms (see Section \ref{sec:sculpting}) to further refine the Concept or Context for future use; if no match is found a new Concept or Context is Acquired (see Section \ref{sec:acquisition}).

The goal of matching is to match a single Concept to an incoming FeatureGroup. In all scenarios, the matching algorithm uses all the Features in the FeatureGroup and the Relationships to other Concepts to identify a matching concept. The Features in the incoming FeatureGroup act as an inverse index to candidate Concepts and their ValueEdges contribute to the likelihood that any candidate Concept is a match to the FeatureGroup.

There are two paths for Matching Concepts and Contexts. The fast path and best case scenario is a top-down mechanism that uses Context to identify Concepts that match the FeatureGroup that has recently arrived from saliency mechanisms. In the fast path a Context has already been identified from previous turns of the agent and there is a variant of the Context (same existing Concepts plus one) that contains the Concept that matches that FeatureGroup. This narrows the scope of searching for Concepts that match FeatureGroups down to a small search area of similar Concepts. 

 In the slow path, and worst case scenario, no Context is available or the Concept isn't associated with any similar Context, the agent uses a bottom-up search of spreading activation across all FeatureGroups. This spreading activation uses the probabilistic edges to find high-confidence matches between the incoming FeatureGroup and Concepts, where the highest confidence match will be established as the new Concept. If no high confidence match is found, a new Concept will be acquired. In the bottom-up approach a new Context will be Acquired that includes the Concept and in the new Context.


\subsection{Acquisition}\label{sec:acquisition}
- At this point we have a stream of concepts from saliency and cross-modal attention; however, it is too much to retain and not all of it is necessary to retain for future purposes
