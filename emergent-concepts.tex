...
Matching is a synonym for recognition or recall --- it reads the ontology graph and returns the best fitting Concept and / or Context for the information that has been made available by saliency and cross-modal attention. The goal of matching is to match an incoming FeatureGroup to a single Concept. In order to create and maintain a structure that accurately represents the world, any FeatureGroup that is not matched is Acquired (see Section \ref{sec:acquisition}) as a new Concept, which makes Matching and Acquisition two halves of the same process. Where a match is found between a FeatureGroup and an existing Concept, it is routed to Sculpting mechanisms (see Section \ref{sec:sculpting}) to further refine the Concept or Context for future use. The matching process that leads to either Acquisition or Sculpting is a single comparison using the mechanism described below.

The matching algorithm uses all the Features in the FeatureGroup and the Relationships to other Concepts to identify a matching concept. The Features in the incoming FeatureGroup act as an inverse index to candidate Concepts each candidate Concept is scored by how well the probabilistic edges describe the Concept versus a reserved probabilistic mass for a new Concept that would be acquired. The reserved mass for acquiring a new Concept is based on $q_f$, the global appearance rates of new features which is stored on each Feature; and $g_{0,f}(x) = \frac{1}{\mathrm{hi}_f - \mathrm{lo}_f}$, the normalized value of the feature such that the maximum entropy of the feature falls within the range $[0,1]$, such that existing Concepts are competing to see if they describe the incoming FeatureGroup better than the background rate of all Features that have been observed globally.

As an optimization, there is also a fast-path for assembling a candidate group of Concepts. The fast path is top-down, using the currently active Context to prime the Concepts that it expects to be present.

Each Feature in the detected FeatureGroup contributes to a candidate Concept's score:

$$
s_f(C) \;=\;
\begin{cases}
\log\dfrac{a/(a+b)}{q_f} \;+\; \log\dfrac{\mathrm{St}\!\left(x_f \mid \mu,\kappa,\alpha,\beta\right)}{g_{0,f}(x_f)} & \text{expected $f$ present with value $x_f$,}\\[10pt]
\log\dfrac{1 - a/(a+b)}{1 - q_f} & \text{expected $f$ absent,}\\[10pt]
0 & \text{$f$ unknown to both hypotheses}
\end{cases}
$$

In the first case, a feature contributes more to a Concept's candidate score based on: the more strongly the Concept $C$ expects the feature than the background rate of the feature in the world (criteriality $a/(a+b)$ against the background rate $q_f$) and the better that the observed value fits the edge's Student-$t$ predictive $\mathrm{St}(\cdot \mid \mu,\kappa,\alpha,\beta)$ than a value drawn from anywhere in the extractor's range.
...
\hl{TODO: Features are on FeatureGroups, not Concepts}
...
Finally, in the third case, if $C$ does not expect $f$ it is not charged for it.
...
where $\mathcal{C}$ is the candidate set and $\pi_C$ is a candidate's prior weight
the prior weights are normalized so that $\sum_C \pi_C = 1 - p_{\mathrm{new}}$
A match is declared exactly when some existing Concept outscores $\nu$; a new Concept is acquired exactly when $\nu$ outscores every existing candidate.

Picking up from where the Matching left off, a concept is acquired when the reserved probability mass of a new concept out-performs all existing concepts. That is, no existing structure in the ontology offered a better explanation of the features that were observed than creating a new concept.
